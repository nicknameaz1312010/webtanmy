...
new MutationObserver(function(){
    var b=document.querySelector('.goog-te-banner-frame');if(b)b.style.display='none';
    var t=document.body.style.top;if(t!=='0px'&&t!=='0')document.body.style.top='0px';
}).observe(document,{childList:true,subtree:true,attributes:true,attributeFilter:['style']});
function changeLang(lang){
    var d=new Date();d.setTime(d.getTime()+365*24*60*60*1000);
    if(lang==='en'){document.cookie='googtrans=/vi/en;path=/;expires='+d.toUTCString();}
    else{document.cookie='googtrans=;path=/;expires=Thu, 01 Jan 1970 00:00:00 UTC';}
    location.reload();
}
